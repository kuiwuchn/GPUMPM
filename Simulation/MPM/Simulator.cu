...
namespace mn {
	...
	{
        if (integratorType == 1)
            _pTimeIntegrator = std::make_unique<ImplicitTimeIntegrator>(transferScheme, _numParticle, d_memTrunk);
        else
            _pTimeIntegrator = std::make_unique<ExplicitTimeIntegrator>(transferScheme, _numParticle, d_memTrunk);
        printf("\n4\tFinish Integrator initializing\n");

        _dtDefault = dtDefault;

        configuredLaunch({"InitMatrix", refGeometryPtr(0)->_numParticle}, initMatrix, refGeometryPtr(0)->_numParticle, refGeometryPtr(0)->d_F);
        if (transferScheme)
            checkCudaErrors(cudaMemset(refGeometryPtr(0)->d_B, 0, sizeof(T)*9*refGeometryPtr(0)->_numParticle));
    ...
    T MPMSimulator::computeDt(const T cur, const T next) 
	{
        ...
        checkCudaErrors(cudaMemcpy(d_maxVelSquared, (void*)&maxVel, sizeof(T), cudaMemcpyHostToDevice));
        configuredLaunch({"CalcMaxVel", refGeometryPtr(0)->_numParticle }, calcMaxVel, refGeometryPtr(0)->_numParticle,(const T **) refGeometryPtr(0)->d_orderedVel, d_maxVelSquared);
        checkCudaErrors(cudaMemcpy((void*)&maxVel, d_maxVelSquared, sizeof(T), cudaMemcpyDeviceToHost));
    ...
    void MPMSimulator::simulateToTargetTime(const T targetTime) 
	{
        ...
        do {
            ...
            printf("\n\t\t\tSubstep %d [%.6f, %.6f] dt: %.6f\n", substep++, _currentTime, _currentTime + dt, dt);
	...
	{
        getTimeIntegratorPtr<MPM_SIM_TYPE>()->integrate(dt, refModel(0), refGridPtr(), refTransformerPtr());
    }
}
