...
namespace mn {
    ...
    std::string getMaterialName(int opt) {
        if (opt == 0) return "neohookean";
        else if (opt == 1) return "fixedcorotated";
        else if (opt == 2) return "newSand";
        else if (opt == 3) return "snow";
        else return "unknownmaterial";
    }
    std::string getTransferScheme(int opt) {
        if (opt == 0) return "flip";
        else if (opt == 1) return "apic";
        else if (opt == 2) return "mls";
        else return "unknown";
    }
    ...
    bool SimulatorBuilder::build(MPMSimulator& simulator, int opt) {
        ...
        if (opt == 0) {
            ...
            std::fill(simulator.h_vel.begin(), simulator.h_vel.end(), std::array<T, Dim>{0, 0, 0});
            ...
            for (int i = 0; i < simulator._numParticle; i++)
                if (simulator.h_pos[i][2] > 0.5f) numPartParticle++;
            {
                std::cout << "The Number of Particles for the first part" << numPartParticle << std::endl;
                ...
                for (int i = 0; i < simulator._numParticle; i++)
                    if (tmp[i][2] > 0.5) {
                        simulator.h_pos[cubeIndex] = tmp[i]; simulator.h_vel[cubeIndex++][2] = -0.5;
                    }
                    else {
                        simulator.h_pos[baseIndex] = tmp[i]; simulator.h_vel[baseIndex++][2] = +0.5;
                    }
                tmp.clear();
            }
        ...
        for (int i = 0; i < simulator._numParticle; ++i) simulator.h_indices[i] = i;
        ...
        for (auto& mass : simulator.h_mass) mass = pMass;

        //////////////////////////////////////////////////////////////////////////
        // file title
        char str[500];
        sprintf(str, "%s%s_initialize(%d)_%s_par[%d]_grid[%d]_%s_%e_", 
            prefix.data(), getTransferScheme(transferScheme).data(), opt, getMaterialName(materialType).data(), 
            simulator._numParticle, N, 
            integratorType ? "imp" : "exp", dtDefault);
        simulator._fileTitle = std::string(str);

        ///
        reportMemory("before simulator_allocation");
        checkCudaErrors(cudaMalloc((void**)&simulator.d_memTrunk,			sizeof(T) * simulator._numParticle * (21))); // tmp(21)         
        ...
        checkCudaErrors(cudaMemcpy(simulator.d_masks, simulator.h_masks.data(),		sizeof(uint64_t) * Dim, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMalloc((void**)&simulator.d_keyTable,			sizeof(uint64_t) * simulator._tableSize));
        checkCudaErrors(cudaMalloc((void**)&simulator.d_valTable,			sizeof(int) * simulator._tableSize));
        reportMemory("after simulator_allocation");

        /// components
        printf("volume is %f\n",volume);
        if(materialType==0 || materialType==1)
            simulator.buildModel(simulator.h_mass, simulator.h_pos, simulator.h_vel, materialType, youngsModulus, poissonRatio, density, volume, 0);
