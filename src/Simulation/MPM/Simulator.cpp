...
namespace mn {
    ...
    MPMSimulator::MPMSimulator() : _frameRate(48.f), _currentFrame(0), _currentTime(0) {}
    ...
    void MPMSimulator::buildModel(
        std::vector<T> &hmass, std::vector<std::array<T, Dim>> &hpos, std::vector<std::array<T, Dim>> &hvel, const int materialType,
        const T youngsModulus, const T poissonRatio, const T density, const T volume,
        int idx) 
    {
        {
            auto geometry = std::make_unique<Particles>(_numParticle, _tableSize);
            auto material = std::make_unique<ElasticMaterialDynamics>(materialType, youngsModulus, poissonRatio, density, volume);
            h_models.emplace_back(std::make_unique<Model>(std::move(geometry), std::move(material)));
            printf("\n\tFinish Particles initializing\n");
        }

        auto& geometry = refGeometryPtr(idx);
        geometry->initialize(hmass.data(), (const T*)hpos.data(), (const T*)hvel.data(), d_masks, h_masks.data(), 
