...
namespace mn {
		...
		, _tableSize(tableSize) 
	{
        ...
        checkCudaErrors(cudaMalloc((void**)&d_indexTrans,	sizeof(int) * _numParticle));

        checkCudaErrors(cudaMalloc((void**)&d_numBucket, sizeof(int)));
        checkCudaErrors(cudaMalloc((void**)&d_particle2bucket, sizeof(int) * _numParticle));
        checkCudaErrors(cudaMalloc((void**)&d_bucketSizes, sizeof(int) * _tableSize));
        checkCudaErrors(cudaMalloc((void**)&d_bucketOffsets, sizeof(int) * _tableSize));
        ...
        checkCudaErrors(cudaMalloc((void**)&d_smallestNodeIndex,	sizeof(int*) * Dim));

        checkCudaErrors(cudaMemcpy(d_pos, hd_pos,					sizeof(T*) * Dim, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_orderedPos, hd_orderedPos,		sizeof(T*) * Dim, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_orderedVel, hd_orderedVel,		sizeof(T*) * Dim, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_smallestNodeIndex, hd_smallestNodeIndex,	sizeof(int*) * Dim,			cudaMemcpyHostToDevice));
