...
namespace mn {
    ...
    __global__ void calcOffset(const int numParticle, const T one_over_dx, const ulonglong3 _masks, T** _pos, uint64_t* _offsets) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= numParticle) return;
        _offsets[idx] |= Bit_Spread_Mine(_masks.x, (int)((_pos[0][idx]) * one_over_dx + 0.5f) - 1);
        _offsets[idx] |= Bit_Spread_Mine(_masks.y, (int)((_pos[1][idx]) * one_over_dx + 0.5f) - 1);
        _offsets[idx] |= Bit_Spread_Mine(_masks.z, (int)((_pos[2][idx]) * one_over_dx + 0.5f) - 1);
    }

    __global__ void gather3D(int numParticle, const int* _map, const T** _ori, T** _ord) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= numParticle) return;
        for(int i = 0; i < 3; i++)
    ...
    __global__ void gather3DShared(int numParticle, const int* _map, const T** _ori, T** _ord) {
