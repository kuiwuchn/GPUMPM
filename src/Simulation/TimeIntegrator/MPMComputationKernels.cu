...
namespace mn {
    ...
    __global__ void SVD3_Pre_Calculation(int numparticle, float* input, float* ouputdata) {
	    int tid = blockDim.x * blockIdx.x + threadIdx.x;

	    if (tid >= numparticle) return;

	    int threadPerBlock = min(blockDim.x, numparticle);

	    if (tid >= numparticle / blockDim.x * blockDim.x) threadPerBlock = numparticle % blockDim.x;
	    ...
	    svd(sArray[threadIdx.x * 21 + 0].f, sArray[threadIdx.x * 21 + 3].f, sArray[threadIdx.x * 21 + 6].f,
		    sArray[threadIdx.x * 21 + 1].f, sArray[threadIdx.x * 21 + 4].f, sArray[threadIdx.x * 21 + 7].f,
		    sArray[threadIdx.x * 21 + 2].f, sArray[threadIdx.x * 21 + 5].f, sArray[threadIdx.x * 21 + 8].f,
		    sArray[threadIdx.x * 21 + 0].f, sArray[threadIdx.x * 21 + 3].f, sArray[threadIdx.x * 21 + 6].f,
		    sArray[threadIdx.x * 21 + 1].f, sArray[threadIdx.x * 21 + 4].f, sArray[threadIdx.x * 21 + 7].f,
		    sArray[threadIdx.x * 21 + 2].f, sArray[threadIdx.x * 21 + 5].f, sArray[threadIdx.x * 21 + 8].f,
		    sArray[threadIdx.x * 21 + 9].f, sArray[threadIdx.x * 21 + 10].f, sArray[threadIdx.x * 21 + 11].f,
		    sArray[threadIdx.x * 21 + 12].f, sArray[threadIdx.x * 21 + 15].f, sArray[threadIdx.x * 21 + 18].f,
		    sArray[threadIdx.x * 21 + 13].f, sArray[threadIdx.x * 21 + 16].f, sArray[threadIdx.x * 21 + 19].f,
		    sArray[threadIdx.x * 21 + 14].f, sArray[threadIdx.x * 21 + 17].f, sArray[threadIdx.x * 21 + 20].f
	    );
    ...
    __global__
    void collideWithGround(ulonglong3 masks,
                                 ...
                                 T** d_channels)
    {
        ...
        // sticky
        if(i<=4) if(*((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) < 0.f){
        ...
        if(i>N-4) if(*((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) > 0.f){
            *((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) = 0.f;
            }
        if(j<=4) if(*((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) < 0.f){
            *((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) = 0.f;
            }
        if(j>N-4) if(*((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) > 0.f){
            *((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) = 0.f;
            }
        if(k<=4) if(*((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) < 0.f){
            *((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) = 0.f;
            }
        if(k>N-4) if(*((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) > 0.f){
            *((T*)((uint64_t)d_channels[3] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[1] + blockIdx.x * 4096) + cell) = 0.f;
            *((T*)((uint64_t)d_channels[2] + blockIdx.x * 4096) + cell) = 0.f;
            }
